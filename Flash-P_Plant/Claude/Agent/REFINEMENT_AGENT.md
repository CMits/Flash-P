...
|---|---|---|
| ratio \u2248 1.0 (within 0.99\u20131.01) | **No signal propagated.** Geometric-mean dilution at a hub (perturbed gene fed a 10+-input node), or the inhibition term already sitting at its 2.0 ceiling (another inhibitor of the same node is already knocked out, so removing this one changes nothing) | Fixable for hub dilution (trim hub inputs); the ceiling case needs the co-inhibitors re-examined, since only one of them can register |
| ratio > 5 or < 0.2 | **Cascade amplification.** A perturbed gene caused a single-input-only downstream node to drop to the activator_floor (0.01), which then propagates through an inhibition step. Uncommon, since the inhibition term caps de-repression at 2.0 rather than letting it run away | Fixable: add another activator to the collapsed node so the perturbation can't drive it to floor |
...
- **Cluster 2: inhibition ceiling in BR-rescue tests.** Affects T063, T064, T065 (all ratio=1.0 with mutant baseline). Root cause: the det2 baseline already knocks out one inhibitor of the target node, so `2/(1 + product)` is pinned at 2.0 and removing a second inhibitor cannot move it further. **Fixable:** route the second effect through a different node, or check whether both really inhibit the same target.
...
**Does NOT:** rebuild the network from scratch (BUILDER's job), modify the equation framework (geometric-mean activation, soft-bounded inhibition, Hill functions, RWR are fixed), tune K/n/alpha (validator sensitivity sweeps do that), ignore failures without justification, add edges without DOI evidence, invent mechanisms not in literature, chase accuracy past the +0.5%-per-iteration threshold.
...
{
  "metadata": {
    "flash_p_version": "<run: python Agent/shared/flashp_version.py --bare>", "build_variant": "debiasing", "phenotype": "shoot_branching",
