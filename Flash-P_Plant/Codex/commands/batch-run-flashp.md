# Codex Recipe: batch-run-flashp

Use when the user asks for `/batch-run-flashp`, batch FLASH-P runs, running a phenotype list, `--skip-existing`, or interval/wave scheduling.
...
Claude batch mode used `Agent/shared/batch_run_flashp.py` and launched `claude -p`. Codex batch mode must use `Agent/shared/codex_batch_run_flashp.py`, which launches `codex exec`.
