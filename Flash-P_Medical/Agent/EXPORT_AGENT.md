...
4c. **Studio refresh.** `python Agent/shared/network_to_studio.py "<networks_dir>"` where `<networks_dir>` is the **parent** folder containing all trait networks (the directory holding `{network}`, normally `networks`). Rebuilds `<networks_dir>/Flash-P_Studio.html` — one self-contained, offline HTML app embedding **every** network so the just-exported readout is added automatically and the user can browse, view (DOIs), and **perturbate** all networks (KO/KD/OE + treatments; Algebraic / RWR / ODE) by double-click. By default the refreshed Studio **auto-opens in the browser** when this final step finishes so the user sees the just-built network immediately (pass `--no-open` to suppress it in headless/scripted runs). Never fails the export; note it was refreshed and how many networks it embedded.
